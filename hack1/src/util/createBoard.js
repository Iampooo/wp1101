...
export default (boardSize, mineNum) => {
    ...
    for(let x = 0; x < boardSize; x++){
        for(let y = 0; y < boardSize; y++){
            ...
            if(x>0 && y<boardSize-1&& board[x-1][y+1].value==='💣'){
                board[x][y].value++;
            }
            if(y<boardSize-1 && board[x][y+1].value==='💣'){
                board[x][y].value++;
            }
            if(x<boardSize-1&&y<boardSize-1 && board[x+1][y+1].value==='💣'){
                board[x][y].value++;
            }
            if(x<boardSize-1 && board[x+1][y].value==='💣'){
                board[x][y].value++;
            }
            if(x<boardSize-1&&y>0 && board[x+1][y-1].value==='💣'){
                board[x][y].value++;
            }
            if(y>0 && board[x][y-1].value==='💣'){
                board[x][y].value++;
            }
            if(x>0 && y>0 && board[x-1][y-1].value==='💣'){
                board[x][y].value++;
            }
        }
    }
